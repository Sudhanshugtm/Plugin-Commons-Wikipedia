...
const INTER_ITALIC = { family: 'Inter', style: 'Italic' };
const INTER_BOLD_ITALIC = { family: 'Inter', style: 'Bold Italic' };
const LINK_PAINT = {
    type: 'SOLID',
...
async function runWithLoading(operation, errorMessage) {
    setLoading(true);
    try {
        await operation();
    }
...
figma.ui.onmessage = async (message) => {
    switch (message.type) {
        ...
        case 'insert-image': {
            await runWithLoading(() => handleInsertImage(message), 'Failed to insert image');
            break;
        }
        case 'insert-text': {
            await runWithLoading(() => handleInsertText(message), 'Failed to insert text');
            break;
        }
        case 'insert-formatted-text': {
            await runWithLoading(() => handleInsertFormattedText(message), 'Failed to insert formatted text');
            break;
        }
        default: {
...
async function handleInsertImage(message) {
    ...
    const imagePaint = { type: 'IMAGE', scaleMode: 'FILL', imageHash };
    const selection = figma.currentPage.selection.filter((node) => isFillableNode(node));
...
async function handleInsertText(message) {
    ...
    const textNodes = figma.currentPage.selection.filter((node) => node.type === 'TEXT' && !node.locked);
...
async function handleInsertFormattedText(message) {
...
async function prepareTextNode(textNode) {
...
function applyDefaultTextWidth(textNode) {
...
async function insertFormattedTextIntoNode(textNode, richText, applyDefaultWidth) {
    var _a;
    ...
    const boldItalicAvailable = regularLoaded && (await ensureFont(INTER_BOLD_ITALIC));
    ...
    textNode.fontSize = DEFAULT_FONT_SIZE;
    textNode.textAutoResize = 'HEIGHT';
    const processedSegments = richText.map((segment) => {
    ...
    for (const segment of processedSegments) {
        ...
        const headingLevel = (_a = segment.format.headingLevel) !== null && _a !== void 0 ? _a : null;
...
function getRangeBaseFontSize(textNode, start, end) {
...
function resolveFontForSegment(format, availability) {
...
async function ensureFont(font, required = false) {
